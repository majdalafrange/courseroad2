...
/**
 * One preset in a GColorSwatchPickerRoot. `value` is the item's identity
 * and (unless `color` overrides it) its fill. The fill and check mark are
 * grandchildren from the caller's view, so a caller's class on them
 * needs :deep().
 */
