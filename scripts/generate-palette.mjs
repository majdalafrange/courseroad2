 ...
 *
 * Bands hold ≥6.3:1 for --dept-on, not just the ≥4.5:1 AA floor: a
 * department color is now a full card fill, not only a rest-state rail,
 * so --dept-on-2/-3 (lower-emphasis text on that fill, e.g. a card's
 * title under its id) need headroom below the primary color and still
 * have to clear 4.5:1 themselves. They're plain opaque greys rather than
 * --dept-on faded toward transparent, which fades toward the fill's own
 * hue and can lose contrast on the wrong department.
 *
 * Light theme: L=0.46/0.45 (main/deep) C=0.115/0.105, white text
 * Dark theme:  L=0.76/0.72 (main/deep) C=0.095/0.10,  ink text
 *
...
/* ---- Theme bands ---- */
const LIGHT = { L: 0.46, C: 0.115 };
const LIGHT_DEEP = { L: 0.45, C: 0.105 };
const DARK = { L: 0.76, C: 0.095 };
const DARK_DEEP = { L: 0.72, C: 0.1 };

/** @type {[number, number, number]} */
const WHITE = [1, 1, 1];
const INK = oklchToRgb(0.22, 0.012, 250); // --g-ink dark text

/* Secondary/tertiary on-color text for a department fill: fixed opaque
   greys (not --dept-on mixed toward transparent, which fades toward the
   fill's own hue and can crash contrast on the wrong department), each
   verified below to clear 4.5:1 against every department in both tiers. */
const DEPT_ON_2_LIGHT = "#f0f0f0";
const DEPT_ON_3_LIGHT = "#eaeaea";
const DEPT_ON_2_DARK = "#2a2a2a";
const DEPT_ON_3_DARK = "#323232";
...
/* "No department" fallback grey; distinct from the ROTC tints below, as
   in the legacy app (#999999 vs #b0b0b0). Darker than the old #8a8f98
   (3.25:1 with white text) because it's a full-card fill now, same as
   every real department, and needs the same 4.5:1. Same L as
   NEUTRAL_LIGHT_L below, C=0. */
const NEUTRAL = {
  light: "#666666",
  dark: "#9aa0a8",
};
...
/* Lightness matching NEUTRAL at C≈0, so ROTC tints sit at the same
   visual weight instead of jumping to the department L/C bands (light is
   darker than "faint" suggests, for the same reason as NEUTRAL above). */
...
/**
 * Parses a "#rrggbb" hex string to sRGB in [0, 1].
...
/**
 * Fails loudly if a fixed on-color text (secondary/tertiary, opaque, not
 * per-department) drops below AA against any generated department fill.
 * @param {string} label Text token name, for the error message
 * @param {string} onHex The fixed opaque color
 * @param {"light" | "dark"} themeKey "light" or "dark"
 */
function assertOnColorContrast(label, onHex, themeKey) {
  const on = fromHex(onHex);
  let min = Infinity;
  let worstDept = "";
  for (const [dept, value] of Object.entries(entries)) {
    const c = contrast(on, fromHex(value[themeKey]));
    if (c < min) {
      min = c;
      worstDept = dept;
    }
  ...
  if (min < 4.5) {
    throw new Error(
      `${label} (${onHex}) only clears ${min.toFixed(2)}:1 against ${themeKey} ` +
        `--dept-${worstDept} (${entries[worstDept][themeKey]}); needs ≥4.5:1. ` +
        `Widen the ${themeKey} band(s) or adjust ${label}.`,
    );
  }
  console.log(
    `${label}: ${min.toFixed(2)}:1 worst-case (${themeKey} --dept-${worstDept})`,
  );
}
assertOnColorContrast("--dept-on-2-light", DEPT_ON_2_LIGHT, "light");
assertOnColorContrast("--dept-on-3-light", DEPT_ON_3_LIGHT, "light");
assertOnColorContrast("--dept-on-2-dark", DEPT_ON_2_DARK, "dark");
assertOnColorContrast("--dept-on-3-dark", DEPT_ON_3_DARK, "dark");
...
css += `  --dept-on: #ffffff;\n`;
css += `  --dept-on-2: ${DEPT_ON_2_LIGHT};\n`;
css += `  --dept-on-3: ${DEPT_ON_3_LIGHT};\n}\n\n[data-theme="dark"] {\n`;
for (const [key, { dark }] of Object.entries(entries)) {
  css += `  --dept-${key}: ${dark};\n`;
}
css += `  --dept-on: #16191d;\n`;
css += `  --dept-on-2: ${DEPT_ON_2_DARK};\n`;
css += `  --dept-on-3: ${DEPT_ON_3_DARK};\n}\n`;
