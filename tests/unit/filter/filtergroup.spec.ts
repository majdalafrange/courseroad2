...
import { FilterGroup, Filter } from "../../../src/lib/filters";
...
describe("FilterGroup", () => {
  ...
  */
  ...
  it("passes AND group with 0 filters", () => {
    ...
    expect(g.matches({ x: 9, y: 10, z: "hi" }, [])).toBe(true);
  });
  ...
  it("passes OR groups with 0 active filters", () => {
    ...
    const c = new Filter("testc", "c", (x: number) => x === 8, ["d"], "AND");
    const g = new FilterGroup("test", [a, b, c], "OR");
    expect(
      g.matches({ a: "cat", b: "scoot", c: "a", d: 10 }, [false, false, false]),
    ).toBe(true);
  });
  ...
  it("passes AND groups with 0 active filters", () => {
    ...
    const c = new Filter("testc", "c", (x: number) => x < 3, ["b", "c"], "AND");
    const g = new FilterGroup("test", [a, b, c], "AND");
    expect(g.matches({ a: 6, b: 2, c: 4, d: 4 }, [false, false, false])).toBe(
      true,
    );
  });
  ...
  it("passes matching OR groups with some active filters", () => {
    ...
    const c = new Filter("testc", "c", (x: number) => x > 0, ["a", "d"], "AND");
    const d = new Filter(
      "testd",
      "d",
      (x: number) => x % 4 === 0,
  ...
  it("fails non-matching OR groups with some active filters", () => {
    ...
    const g = new FilterGroup("test", [a, b, c, d], "OR");
    expect(
      g.matches({ a: "cat", b: "corn", c: "big", d: "bat" }, [
        true,
        false,
        true,
        true,
  ...
  it("fails non-matching AND groups with some active filters", () => {
    ...
    const g = new FilterGroup("test", [a, b, c, d], "AND");
    expect(
      g.matches({ a: "red", b: "green", c: "blue", d: "yellow" }, [
        true,
        false,
        false,
        true,
  ...
  it("passes matching OR groups with all filters active", () => {
    ...
    const b = new Filter("testb", "b", (x: number) => x > 7, ["b", "c"], "OR");
    const c = new Filter(
      "testc",
      "c",
      (x: number) => x <= 5,
  ...
  it("fails non-matching OR groups with all filters active", () => {
    ...
    const g = new FilterGroup("test", [a, b, c], "OR");
    expect(
      g.matches({ a: "cat", b: "bat", c: "rat", d: "at" }, [true, true, true]),
    ).toBe(false);
  });
  ...
  it("fails non-matching AND groups with all filters active", () => {
    ...
    const g = new FilterGroup("test", [a, b, c], "AND");
    expect(g.matches({ a: -4, b: 14, c: 10, d: -1 }, [true, true, true])).toBe(
      false,
    );
  });
});
