...
import { RegexFilter } from "../../../src/lib/filters";

function expectRegexFilterOrder(
...
describe("RegexFilter", () => {
  ...
  */
  ...
  it("compiles over-length input as an escaped literal (backtracking cap)", () => {
    const pathological = "(a+)+$".repeat(60); // 360 chars, past the cap
    expect(pathological.length).toBeGreaterThan(RegexFilter.MAX_REGEX_INPUT);
    const start = Date.now();
    const regex = RegexFilter.constructRegex(pathological);
    // Literal mode: matches its own (truncated) text, not as a pattern.
    expect(regex.test(pathological)).toBe(true);
    expect(regex.test("aaaa")).toBe(false);
    expect(regex.source).not.toContain("(a+)+");
    expect(Date.now() - start).toBeLessThan(1000);
  });

  it("still compiles short input as a live pattern", () => {
    const regex = RegexFilter.constructRegex("6\\.\\d+");
    expect(regex.test("6.006")).toBe(true);
  });
});
