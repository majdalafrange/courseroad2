...
/**
 * Constructor shape the harness drives: name and short name, then
 * filter-specific arguments, then attributeNames and mode (the same
 * forwarding contract as SubfilterConstructor in filters.ts).
 */
type TestableFilterCtor = (new (
  name: string,
  shortName: string,
  ...args: never[]
) => AnyFilter) & { readonly name: string };

function testFilter(
  FilterType: TestableFilterCtor,
  args: readonly unknown[],
  matching: readonly unknown[],
  nonmatching: readonly unknown[],
) {
  // One cast forwards the per-class argument list; each concrete
  // constructor typechecks its own direct call sites elsewhere.
  const build = (attributeNames: string[], mode: "AND" | "OR"): AnyFilter =>
    new FilterType(
      "test",
      "t",
      ...([...args, attributeNames, mode] as never[]),
    );
...
testFilter(Filter, [(x: unknown) => x === 7], [7], [4, 6, 13, "cat", "dog"]);
testFilter(
  RegexFilter,
  ["^ca.+", ""],
